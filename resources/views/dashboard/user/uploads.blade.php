...
@section('dashboard_content')
    <div class="container px-0 py-0 mx-0 my-0">
        <div class="row">
            ...
            <div>
                <a class="btn float d-flex align-items-center justify-content-center" href="{{ route('upload_file') }}"
                    role="button">
                    <i class="bi bi-upload" style="font-size: 150%;"></i>
                </a>
            </div>
        </div>
    </div>
...
@section('bottomScript')
    ...
    <script>
        ...
        function loadModels(page) {
            $.ajax({
                url: '/fetchUploadModels?page=' + page,
                method: 'GET',
                success: function(data) {
                    ...
                    $.each(models, function(index, model) {
                        let isPrivate = model.private; // replace this with your actual condition
                        let iconClass = isPrivate ? "bi bi-lock-fill" : "bi bi-unlock-fill";
                        var modelHtml = `
                        <!-- The Card -->
                        <div class="col">
                            <div class="card px-0 py-0 shadow-sm rounded-3">
                                ...
                                <div class="card-body">
                                    ...
                                    <h5 class="card-title fw-medium poppins-regular">${model.name}  <i class="${iconClass} px-0 py-0 mx-0 my-0" style="font-size: 85%"></i></h5>
